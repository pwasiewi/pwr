...
src_unpack() {
	...
	mkdir -p "${S}/wheel" || die
	# ${A} is a space-separated list of all SRC_URI basenames; iterate it
	# UNQUOTED so word-splitting yields one wheel per iteration. Quoting it
	# ("${A}") collapses every wheel into a single argument with embedded
	# spaces, so cp gets a nonexistent "a.whl b.whl" path and dies.
	local f
	for f in ${A}; do
