...
src_unpack() {
	...
	mkdir -p "${S}/wheel" || die
	local f
	# ${A} unquoted on purpose: it is a space-separated LIST. Quoted, both
	# wheels arrive as one word and cp dies — latent wherever a single
	# PYTHON_TARGETS is enabled, fatal with two (nvidiahard, 2026-08-08).
	for f in ${A}; do
