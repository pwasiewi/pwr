...
# ::gentoo gui-wm/gamescope-3.16.25-r1 bumped to 3.16.28 (2026-08-27): the only
# build-system change between the tags is wlroots 0.19 -> 0.20; reshade,
# vkroots and libliftoff (v0.5.0 = 8b08dc1c) submodules are unchanged and both
# tree patches still apply (stb hunk with offset).
# libdisplay-info is BUNDLED here (submodule 47a5590e): upstream pins
# '< 0.4.0' and the tree's unbundling would downgrade the host's 0.4.0 and
# rebuild mesa + kwin against it. Static subproject, not installed
# (--skip-subprojects), exactly like libliftoff/vkroots.
...
EGIT_SUBMODULES=( src/reshade subprojects/{libliftoff,vkroots,libdisplay-info} )
...
if [[ ${PV} == "9999" ]]; then
	...
	SRC_URI="
		...
		https://gitlab.freedesktop.org/emersion/libdisplay-info/-/archive/${LIBDISPLAY_INFO_COMMIT}/libdisplay-info-${LIBDISPLAY_INFO_COMMIT}.tar.gz
	"
	KEYWORDS="~amd64"
fi
...
src_prepare() {
	...
	if [[ ${PV} != "9999" ]]; then
		...
		mv "../libdisplay-info-${LIBDISPLAY_INFO_COMMIT}" subprojects/libdisplay-info || die
		local dir name commit
		for dir in "${EGIT_SUBMODULES[@]}"; do
			[[ ${dir} == */libdisplay-info ]] && continue
			rmdir "${dir}" || die
			name=${dir##*/}
			commit=${name^^}_COMMIT
