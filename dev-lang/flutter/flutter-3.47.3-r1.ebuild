...
bin/cache/dart-sdk is a symlink to /usr/lib/dart, so a copy stays
...
src_install() {
	# pkgcheck IndirectInherits: toolchain-funcs on the rm line is a known false
	# positive of its bash parser; do not add the inherit.
	use examples || rm -r "${S}"/examples || die
	...
	# Symlink into the system Dart SDK installed by dev-lang/dart.
	dosym -r /usr/lib/dart "/opt/${PN}/bin/cache/dart-sdk"
